...
import cv2
import json
from gta_utils import read_depthmap, get_focal_length, LIMBS
...
def create_skeleton_viz_data(nskeletons, njoints):
    ...
    for i in range(nskeletons):
        ...
        single_color[:] = [0., float(i)/nskeletons, 1.]
...
def vis_skeleton_mesh(rec_idx, f_id):
    ...
    for i in range(f_id-90, f_id+90, 10):
        ...
        if os.path.exists(fname):
            ...
            mask_dilation = cv2.dilate(mask.astype(np.uint8), kernel, iterations=1)
            ...
            rgbd_image = o3d.geometry.create_rgbd_image_from_color_and_depth(
                color_raw, depth, depth_scale=1.0, depth_trunc=15.0, convert_rgb_to_intensity=False)
            pcd = o3d.geometry.create_point_cloud_from_rgbd_image(
                rgbd_image,
                o3d.camera.PinholeCameraIntrinsic(
                    PinholeCameraIntrinsic(1920, 1080, focal_length, focal_length, 960., 540.)))
    ...
    for j in range(joints.shape[0]):
        ...
        sphere.paint_uniform_color([0., float(j // jn)/nskeletons, 1.])
...
if __name__ == '__main__':
    ...
    parser.add_argument("-f", "--frame", default=180, type=int, help="frame to visualize")
    args = parser.parse_args()

    vis_skeleton_mesh(args.path + '/', args.frame)
