...
import pickle
import os
from gta_utils import read_depthmap, LIMBS
...
def single_vis(args):
    ...
    for idx in range(30, len(info)):
        if os.path.exists(os.path.join(args.path, '{:05d}'.format(idx) + ".jpg")):
            keypoint = joints_2d[idx]

            # root 
            root_pos = (int(keypoint[14,0]), int(keypoint[14,1]))
            # color image
            frame = cv2.imread(os.path.join(args.path, '{:05d}'.format(idx) + ".jpg"))
            ...
            plt.imshow(100. / depthmap[:, :, 0])
            plt.colorbar()
            plt.savefig(os.path.join(args.outpath, str(idx) + "_vis.jpg"))
            # plt.pause(1)
            plt.close()
