...
import numba
import pickle
from gta_utils import read_depthmap
import cv2
...
def get_cam_extr(cam_pos, cam_rot):
    ...
    pose = Pose(cam_pos, cam_rot/180.*np.pi)
    cam_extr = np.eye(4)
...
for data_path in rec_inds:
    ...
    for i in range(len(info)):
        ...
        if os.path.exists(prefix + '_final.jpg') and os.path.exists(prefix + '_depth.png'):
            os.rename(prefix + '_final.jpg', data_path + '{:05d}'.format(count) + '.jpg')
            os.rename(prefix + '_depth.png', data_path + '{:05d}'.format(count) + '.png')
            os.rename(prefix + '_id.png', data_path + '{:05d}'.format(count) + '_id.png')
            count = count + 1
            ...
            focal_length = get_focal_length(cam_near_clip, cam_field_of_view)
            intrinsic = np.asarray([[focal_length, 0, 960.], [0, focal_length, 540.], [0, 0, 1]])
            cam_extr_ref = get_cam_extr(infot['cam_pos'], infot['cam_rot'])

            joints = np.asarray(keypoint)
            jn = joints.shape[0]
            
            joints_world = np.concatenate([joints, np.ones((jn, 1))], axis=-1)
            joints_cam = joints_world.dot(np.linalg.inv(cam_extr_ref.T))[:, :3]
            joints_2d = np.matmul(intrinsic, joints_cam.T)
            joints_2d = joints_2d[0] / joints_2d[2], joints_2d[1] / joints_2d[2]
            gta_pose_2d = np.asarray(joints_2d).T.reshape(jn, 2)
            joints_cam = joints_cam.reshape(jn, 3)


            # color = cv2.imread(prefix + '_final.jpg')
            # im_p = deepcopy(gta_pose_2d)
            # for j in range(jn):
            #     color = cv2.circle(color, (int(im_p[j,0]), int(im_p[j,1])), 5, (0, 0, 255), 20)
    ...
    np.savez(data_path + 'info_frames.npz', 
